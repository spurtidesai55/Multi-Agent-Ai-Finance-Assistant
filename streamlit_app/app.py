...
# Config setting for API Key
st.sidebar.header("🔑 API Configuration")
api_key = st.sidebar.text_input(
    "Google Gemini API Key", 
    type="password", 
    value=os.environ.get("GEMINI_API_KEY", ""),
    help="Get a free key from ai.google.dev. We only use this locally."
)
...
if api_key:
    ...
    try:
        ...
        # We try to update .env so it persists across runs
...
if not api_key:
    st.sidebar.warning("⚠️ Please provide a Gemini API Key to enable AI analysis. Otherwise, a templated fallback is used.")
